...
with keep.running():
	...
	# 	sqlquery = 'select * from ' + sqltblname + ' order by symbolid, Date asc'
	...
	# drop all rows where a value is 0 so it will result in infinte
	...
	# now subset the data for training

	train_df = fulldf[fulldf.symbolid == 'AAPL']
	train_df[['Open', 'High', 'Low', 'Close', 'AdjClose', 'Volume']] = train_df[['Open', 'High', 'Low', 'Close', 'AdjClose', 'Volume']].shift(-1)
	...
	for episode in range(num_episodes):
		...
		print(f'Step: {env.current_step}, Balance: {env.balance}, Stock Value: {stock_value}, Total Value: {env.balance + stock_value}')

	# Save the trained Q-network
	torch.save(agent.q_network.state_dict(), 'dqn_model.pth')

	# Load the saved Q-network for testing
	...
	# Now subset the data for testing
		...
		while not done:
			...
			stock_value = test_env.shares_held * current_price
